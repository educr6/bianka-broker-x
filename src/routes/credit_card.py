...
def get_credit_card_limit():
    ...
    corex_client = CreditCardsCoreXClient(current_app.config['COREX_BASE_URL'], 2, {"Authorization": "%s %s" % (
        current_app.config['AUTH_HEADER_TYPE'], current_app.config['AUTH_HEADER_VALUE'])})
...
def get_credit_card_transactions():
    ...
    message = transactionsPhraseBuilder.create_transactions_phrase(
...
def get_credit_card_payment_limit_date():
    ...
    alias = request.args.get('alias')
    corex_client = CreditCardsCoreXClient(
        current_app.config['COREX_BASE_URL'], 2, {"Authorization": "%s %s" % (current_app.config['AUTH_HEADER_TYPE'], current_app.config['AUTH_HEADER_VALUE'])})
